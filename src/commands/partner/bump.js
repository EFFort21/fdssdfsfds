...
function bumpLogic (client, message, row, invite) {
            ...
            fields: [
              {
                ...
                value: `Online: \`${guildInfo.online}\` | Idle: \`${guildInfo.idle}\` | DnD: \`${guildInfo.dnd}\` | Invisible (indisponible): \`${guildInfo.invisible}\``,
                inline: false
              },
  ...
  client.embed.send(message, { desc: `Bumped to other guilds!` })
}
...
function getGuildInfo (guild) {
  ...
  members.forEach(member => {
    ...
    if (!member.user.bot && status !== null) {
      ...
      if (status.web === 'invisible' || status.desktop === 'invisible' || status.mobile === 'invisible') {
        invisible++
      }
    } else {
      bots++
